...
public class SendIMSI extends SIMSILowLevel implements MAPServiceOamListener {
	...
	GlobalTitle0100 calledPartyAddress = this.sccpProvider.getParameterFactory().createGlobalTitle(target_msisdn, 0,
		org.mobicents.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, null,
		NatureOfAddress.INTERNATIONAL);
	...
	try {
	    // First create Dialog
	    MAPDialogOam mapDialogOam = this.mapProvider.getMAPServiceOam().createNewDialog(MAPApplicationContext
		    .getInstance(MAPApplicationContextName.imsiRetrievalContext, MAPApplicationContextVersion.version2),
	...
	System.err.printf(
		"[-]Error: DialogReject for DialogId=%d MAPRefuseReason=%s MAPProviderError=%s ApplicationContextName=%s MAPExtensionContainer=%s\n",
	...
	if (logger.isDebugEnabled()) {
	    logger.debug(String.format(
		    "DialogRequest for DialogId=%d DestinationReference=%s OriginReference=%s MAPExtensionContainer=%s\n",
    ...
    public void onDialogUserAbort(MAPDialog mapDialog, MAPUserAbortChoice userReason,
	    MAPExtensionContainer extensionContainer) {
	System.err.printf(
		"033[31m[-]\033[0mError: DialogUserAbort for DialogId=%d MAPUserAbortChoice=%s MAPExtensionContainer=%s\n",
	...
	System.err.printf(
		"\033[31m[-]\033[0mError: ProviderErrorComponent for Dialog=%d and invokeId=%d MAPProviderError=%s\n",
		mapDialog.getLocalDialogId(), invokeId);
	System.exit(1);
    }
    ...
    @Override
    public MAPDialogOam createNewDialog(MAPApplicationContext mapApplicationContext, SccpAddress sccpAddress,
	    AddressString addressString, SccpAddress sccpAddress1, AddressString addressString1, Long aLong)
	    throws MAPException {
	return null;
    }
